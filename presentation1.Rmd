...
#Background image
style_xaringan(
  title_slide_background_image = "img/pngtree-group-of-different-colored-birds-on-a-branch-image_2921675.jpg"
)
```
...
data %>% 
  ...
  summarise(mean_hwi = mean(`Hand-Wing.Index`), sd_hwi = sd(`Hand-Wing.Index`)) %>% 
...
# Conclusion
 ...
 - Beak shapes are not effective explanatory variables based on the models we have created: low r square and high root mean square error values
 - Hand-wing index and mass could be explanatory variables in predicting whether a bird is an invertivore: AUC value of 0.79
 - Low specificity suggests high potential improvements on the model either by adding more predictor or making transformation on data
