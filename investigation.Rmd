...
data_T <- data %>% 
  select(Range.Size,
         ...
         Mass, 
         Centroid.Latitude) %>% 
  mutate(T = case_when(Trophic.Level == "Scavenger" ~ "Yes", 
                       TRUE ~ "No"))
# Modify col names
...
# Construct workflow
data_rec2 <- recipe(Scavenger_10 ~ ., data = data_T) %>% 
  step_naomit(all_predictors()) %>% 
  step_dummy(all_nominal(), -all_outcomes())
...
data_pred2 <- predict(data_fit2, data_test2) %>% 
  bind_cols(data_test2) %>% 
  mutate(T = case_when(Scavenger_10 == "Yes" ~ 1, 
                       TRUE ~ 0,), 
         .pred_class = case_when(.pred_class == "Yes" ~ 1, 
                              TRUE ~ 0)) %>% 
  print(n = 10)
...
```{r limit birds in one region}
amazon_data <- data %>% 
  filter(Centroid.Latitude > -10 & Centroid.Latitude < 10, Centroid.Longitude > -73 & Centroid.Longitude < -40) %>% 
  count()
