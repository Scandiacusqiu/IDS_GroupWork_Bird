...
# Why model does not work?
---
### Plot out the data
```{r plot trophic.niche, echo=FALSE}
data %>% 
  mutate(yn = case_when(Trophic.Niche == "Invertivore" ~ "y", TRUE ~ "n")) %>% 
  ggplot(aes(x = `Hand-Wing.Index`, y = log(Mass), color = yn)) + 
  geom_jitter(alpha = 0.3) + 
  labs(x = "Hand-Wing Index", y = "mass(In Logrithmic)")
```
